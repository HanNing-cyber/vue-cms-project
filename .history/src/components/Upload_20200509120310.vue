...
import axios from '@/plugins/axios'
	export default {
		name:"Upload",
		// 组件外部数据
		props: ['action', 'headers', 'data', 'removeAction', 'src','defaultAvatar','lgImg'],
		// 组件内部数据
		data() {
			return {
		...
		methods: {
			...
			async handleRemove() {
				...
				let{status:md} = await axios.delete(this.removeAction,{params:{src}})
				let{status:lg} = await axios.delete(this.removeAction,{params:{src:lgImg}})
			...
			async handleUpload(e) {
				...
				// });
				let {status,mdImg,lgImg,src} = await axios.post(this.action,formData)
				// 发送ajax
				// var ajax = new XMLHttpRequest();
