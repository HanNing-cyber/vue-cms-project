    ...
    <el-card class="box-card">
      ...
      <el-form :rules="rules" :label-position="labelPosition" ref="form" :model="form" label-width="60px">
        <el-form-item label="账户" required prop="username">
          <el-input v-model="form.username" prefix-icon="el-icon-user" placeholder="用户名长度在3至15个字符！"></el-input>
        </el-form-item>
        <el-form-item label="密码" required prop="password">
          <el-input type="password" v-model="form.password" prefix-icon="el-icon-key" placeholder="密码至少3位数字！"></el-input>
        </el-form-item>
        <el-form-item label="姓名" required prop="fullname">
          <el-input v-model="form.fullname" prefix-icon="el-icon-postcard" placeholder="请填写真实姓名！"></el-input>
        </el-form-item>
        ...
        <el-form-item label="手机" required prop="tel">
          <el-input v-model="form.tel" prefix-icon="el-icon-mobile-phone" placeholder="请填写常用手机号码！"></el-input>
        </el-form-item>
  ...
  methods: {
    submitForm(formName) {
      this.$refs[formName].validate(async(valid) => {
        if (valid) {
          // 校验通过,发送ajax
          let res =await Login.register({...this.form});
          console.log(res);
          if(res.data.status){
