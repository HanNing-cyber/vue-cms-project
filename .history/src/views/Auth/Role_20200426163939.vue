	...
	<div>
		<el-card class="box-card">
			<div slot="header" class="clearfix">
				<span>管理员角色</span>
				<el-button @click="dialogFormVisible = true" class="right" size="small" type="primary" plain><i class="el-icon-circle-plus-outline"></i>
					添加角色</el-button>
			...
			<template>
				<el-table size="mini" :data="tableData" :default-sort="{id:'id',}">
					...
					<el-table-column label="分类" width="230" filter-placement="bottom-end">
					...
					<el-table-column label="操作">
						<template slot-scope="scope">
							<el-button @click="handleEdit(scope.row.id,scope.row.name)" size="mini" type="primary" plain><i class="el-icon-edit"></i> 编辑</el-button>
							<el-button @click="removeHandle(scope.row.id,scope.$index)" size="mini" type="danger" plain><i class="el-icon-delete"></i>
								删除</el-button>
						</template>
					</el-table-column>
				</el-table>
	...
	export default {
		...
		methods: {
			...
			removeHandle(id, index) {
				console.log(id);
				this.$confirm('确定要删除此账户嘛？', { type: 'warning' })
					.then(async () => {
						//发送ajax
			...
			addHandle(newForm) {
				this.$refs[newForm].validate(async (valid) => {
					if (valid) {
						let { status, data } = await Role.add(this.newForm.name)
						if (status) {
			...
			edit(formName) {
				this.$refs[formName].validate(async valid => {
					if (valid) {
						let { status, data, msg } = await Role.edit(this.ID, { ...this.form });
						if (status) {
