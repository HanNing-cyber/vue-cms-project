    ...
    <el-card class="box-card">
      ...
      <el-form ref="form" :model="form" :rules="rules" label-width="80px" size="mini">
        ...
        <el-form-item label="商品卖点" prop="hotPoint">
          <el-input type="textarea" v-model="form.hotPoint" style="width:550px"></el-input>
        ...
        <el-form-item label="商品主图" prop="img_lg">
          <Upload action="/api/upload/goods" remove-action="/api/upload" :lg-img.sync="form.img_lg" :src.sync="form.img_md" :headers="token" list-type="picture-card" :limit="1">
        ...
        <el-form-item label="商品轮播图" prop="img_md">
          <Upload action="/api/upload/slider" remove-action="/api/upload" :lg-img.sync="form.img_lg" :src.sync="form.img_md" :headers="token" list-type="picture-card" :limit="1">
            <i class="el-icon-plus"></i>
          </Upload>
          <div class="sub-title">上传商品默认主图，如多规格时将默认图片或分规格上传规格主图，支持jpg、if、png格式上传或从图片空间选中，建议使用尺寸 800*800像素以上，大小不超过1M的正方形图片，上传后的图片将自动保存在图片空间的默认分类中</div>
        </el-form-item>
  ...
  data() {
    return {
      ...
      rules: {
        ...
        cate_1st: [{ type: "date", required: true, message: "请选择一级分类", trigger: "change" }],
        cate_2nd: [{ type: "date", required: true, message: "请选择二级分类", trigger: "change" }],
        cate_3rd: [{ type: "date", required: true, message: "请选择三级分类", trigger: "change" }],
        marketPrice: [
